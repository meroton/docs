...
When reviving an old module that used to build googletest
I encountered some cpp standard errors.
Could be because we now have a newer googletest version since then
or the shared toolchain was updated.
Either way, we just need t ochange some features around.
...
So I wouldn't have to update the BUILD files manually.

## Patch googletest to use c++17 for its compilation

For this example we don't pay much attention to mixing standards in one compilation.
